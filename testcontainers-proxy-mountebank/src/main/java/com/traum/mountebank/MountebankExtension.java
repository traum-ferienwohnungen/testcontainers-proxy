...
public class MountebankExtension
    implements BeforeEachCallback, AfterEachCallback, ParameterResolver {

  public static final String EXTERNAL_PROXY_PROPERTY_PREFIX = "mountebank.external.proxy";
  public static final String EXTERNAL_PROXY_API_URL_PROPERTY =
      EXTERNAL_PROXY_PROPERTY_PREFIX + ".api.url";

  private static final String STORE_KEY_OUTPUT = "output";
  private static final String STORE_KEY_PROXY = "proxy";

  private static final Pattern PLACEHOLDER_PATTERN = Pattern.compile("\\{([^}]+)\\}");
  private static final Pattern EXTERNAL_PROXY_IMPOSTER_AUTHORITIES =
      Pattern.compile(Pattern.quote(EXTERNAL_PROXY_PROPERTY_PREFIX) + "\\.(\\d+)\\.authority");

  private final Map<String, Function<ExtensionContext, String>> placeholders =
      Map.of(
  ...
  public void beforeEach(ExtensionContext extensionContext) {
    getAnnotation(extensionContext::getTestMethod)
        .ifPresent(
            annotation -> {
              final Optional<Path> replayImposters =
                  getPath(extensionContext, WithProxy::replayImposters);
              final Path importImposters =
                  getPath(extensionContext, WithProxy::initialImposters)
                      .filter(Files::exists)
                      .map(
                          initialPath ->
                              replayImposters
                                      ...
                                      replayPath -> {
                                        try {
                                          if (Files.exists(replayPath)) {
                                            final WithProxy.InitPolicy initPolicy =
                                                getInitPolicy(
                                                    extensionContext, WithProxy::initPolicy);

                                            if (initPolicy
                                                == WithProxy.InitPolicy.IF_REPLAY_OUTDATED) {
                                              return Files.getLastModifiedTime(replayPath)
                                                      .compareTo(
                                                          Files.getLastModifiedTime(initialPath))
                                                  > 0;
                                            }
                                            return initPolicy
                                                == WithProxy.InitPolicy.IF_REPLAY_NONEXISTENT;
                                          }
                                          return false;
                                        } catch (IOException e) {
                                          throw new IllegalArgumentException(
                                              "Failed to compare modification times", e);
                                        }
                                      })
                                  .orElse(initialPath))
                      .orElseGet(
                          () ->
                              replayImposters.orElseThrow(
                                  () ->
                                      new IllegalArgumentException(
                                          "Neither initial nor replay imposters are given or existent")));
              ...
              getProxy(extensionContext, true)
                  .ifPresent(
                      proxy -> {
                        ...
                        replayImposters
                            .filter(Predicate.not(importImposters::equals))
    ...
    getProxy(extensionContext, false)
        .ifPresent(
            proxy -> {
              ...
              getPath(extensionContext, WithProxy::recordImposters)
                  .ifPresent(
                      path -> {
  ...
  @Override
  public MountebankProxy resolveParameter(
      ParameterContext parameterContext, ExtensionContext extensionContext)
      throws ParameterResolutionException {
  ...
  private WithProxy.InitPolicy getInitPolicy(
      ExtensionContext extensionContext,
      Function<WithProxy, WithProxy.InitPolicy> annotationProperty) {
    return getAnnotation(extensionContext::getTestMethod)
        .map(annotationProperty)
        .or(() -> getAnnotation(extensionContext::getTestClass).map(annotationProperty))
        .orElseThrow();
  }

  private Optional<Path> getPath(
      ExtensionContext extensionContext, Function<WithProxy, String> annotationProperty) {
    return getAnnotation(extensionContext::getTestMethod)
        .map(annotationProperty)
        .filter(Predicate.not(String::isBlank))
        .or(
            () ->
                getAnnotation(extensionContext::getTestClass)
                    .map(annotationProperty)
                    .filter(Predicate.not(String::isBlank)))
  ...
  private Optional<WithProxy> getAnnotation(
      Supplier<Optional<? extends AnnotatedElement>> supplier) {
  ...
  public @interface WithProxy {
    ...
    /**
     * Path to ejs/json file. Used to initialize mountebank unless {@link #replayImposters()} is set
     * or is outdated (depending on {@link #initPolicy()}).
     */
    String initialImposters() default "";

    /**
     * Path to ejs/json file. If used with {@link #initialImposters()} this file is overridden with
     * re-play-ready imposters previously recorded by the mountebank proxy.
     */
    String replayImposters() default "";

    /**
     * Path to ejs/json file. If given {@link #replayImposters()} is configured to record requests
     * and the output is stored under the given path.
     */
  ...
  public interface MountebankProxyFactory {
    ...
    default MountebankProxy create() {
      if (System.getProperties().containsKey(EXTERNAL_PROXY_API_URL_PROPERTY)) {
        final HashMap<Integer, String> imposterAuthorities =
            Collections.list(System.getProperties().keys())
                .stream()
                ...
                .map(
                    matcher ->
                        Map.entry(
                            ...
                            System.getProperties().getProperty(matcher.group(0))))
                .reduce(
                    new HashMap<>(),
                    (identity, entry) -> {
        ...
        return new ExternalMountebankProxy(
            System.getProperty(EXTERNAL_PROXY_API_URL_PROPERTY), imposterAuthorities);
      }
